...
from chainerrl.misc.batch_states import batch_states as batch_state
...
def myentropy(self):
    with chainer.force_backprop_mode():
        return functions.stack([- functions.sum(self.all_prob * self.all_log_prob, axis=1)], axis=1)
...
def mylog_prob(self, x):
    ...
    p_trans = functions.transpose(self.all_log_prob, axes=(0, 2, 3, 1))
    p_trans = functions.reshape(p_trans, (-1, n_actions))
    x_reshape = functions.reshape(x, (1, -1))[0]
    selected_p = functions.select_item(p_trans, x_reshape)
    return functions.reshape(selected_p, (n_batch, 1, h, w))
...
class PixelWiseA3C_InnerState_ConvR(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, model, optimizer, t_max, gamma, beta=1e-2,
        ...
        # A3C won't use an explorer, but this attribute is referenced by run_dqn
        self.explorer = None
        ...
        #######################
        self.shared_model.to_device("@cupy:0")
    ...
    def update(self, statevar):
        ...
        if statevar is None:
            ...
            #######################
            # R = functions.cast(vout.data, 'float32')
            R_g = functions.cast(vout.data, 'float32')
            print(R_g)
            # R = float(vout.data)
            #######################
        ...
        for i in reversed(range(self.t_start, self.t)):
            ...
            ##############################
            pi_loss -= log_prob * functions.cast(advantage.data, 'float32')
        ...
        ##########################
        # total_loss = pi_loss + functions.reshape(v_loss, pi_loss.data.shape)
        total_loss = functions.mean(pi_loss + functions.reshape(v_loss, pi_loss.data.shape))
        ##########################

        # Compute gradients using thread-specific model
        self.model.zerograds()  # Cleargrads won't work (outdated version?)
        total_loss.backward()

        # Copy the gradients to the globally shared model
        self.shared_model.zerograds()
        # print("SHARED MODEL", type(self.shared_model))
        # print("MODEL", type(self.model))
        copy_param.copy_grad(target_link=self.shared_model, source_link=self.model)

        # Update the globally shared model
        if self.process_idx == 0:
            norm = sum(np.sum(np.square(param.grad)) for param in self.optimizer.target.params())
    ...
    def act_and_train(self, state, reward):
        ...
        statevar = chainer.backends.cuda.to_gpu(state)

        # self.past_rewards[self.t - 1] = reward
        self.past_rewards[self.t - 1] = chainer.backends.cuda.to_gpu(reward)
        ##########################
        ...
        #    (functions.cast(vout.data, 'float32') - self.average_value))
        #############################
        # (float(vout.data[0]) - self.average_value))
        #############################
        # self.average_entropy += (
        #    (1 - self.average_entropy_decay) *
        #    (functions.cast(pout.entropy.data, 'float32') - self.average_entropy))
        #############################
    ...
    def act(self, obs):
        ...
        with chainer.no_backprop_mode():
            ...
            statevar = chainer.backends.cuda.to_gpu(obs)
